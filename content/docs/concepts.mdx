...
- When you call `menu.openAtElement(el, options)`, the menu is positioned relative to the element (e.g. `bottom-start`), not a raw point.
...
1. **Open** — You trigger the menu via `bind` (right-click/long-press) or by calling `open` / `openAtElement`.
...
<Mermaid chart="flowchart LR\n  A[1. Open<br/>bind · open · openAtElement] --> B[2. Position<br/>anchor · offset · flip] --> C[3. Render<br/>DOM · portal · enter] --> D[4. User action<br/>select · dismiss] --> E[5. Close<br/>leave · unmount · focus]" />
